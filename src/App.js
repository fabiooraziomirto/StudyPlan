...
function App() {
  ...
  const addExam = async (exam) => {
    ...
    if (sP.length != 0) {
      ...
      setCredits(credits + exam.course.credits);
  ...
  const saveStudyPlan = async (sP) => {
    ...
    if(user.enroll === "partTime"){
      if(credits >= 20 && credits <= 40){
        ...
        setMessage({ msg: `TOO MUCH OR TOO MANY`, type: 'danger' });
      }
    } else {
